...
def const(n):
    return n # one of the stupidest things ive ever done
...
class _SCCB16CameraBase:  # pylint: disable=too-few-public-methods
    ...
    def _write_register(self, reg: int, value: int) -> None:
        ...
        #     i2c.write(b)
        self._bank[reg] = value
        self._writes.append( (reg,value) )
        # print("{{{}, {}}},".format(hex(reg),hex(value)))
    ...
    def _read_register(self, reg: int) -> int:
        assert reg in self._bank
        return self._bank[reg]
        # b = bytearray(2)
        # b[0] = reg >> 8
        # b[1] = reg & 0xFF
        # with self._i2c_device as i2c:
    ...
    def _write_reg_bits(self, reg: int, mask: int, enable: bool) -> None:
        ...
        # val = 1 # this is a Stupid solution, for one specific register that gets read (ISP CONTROL 01 default value is 0x01, this gets read in _set_size_and_colorspace
        if enable:
            val |= mask
        else:
            val &= ~mask
...
class OV5640(_SCCB16CameraBase):  # pylint: disable=too-many-instance-attributes
    ...
    def _set_size_and_colorspace(self) -> None:  # pylint: disable=too-many-locals
        ...
        if not self._binning:
            ...
            if width > 920:
                ...
                self._write_addr_reg(_X_TOTAL_SIZE_H, 2060, 984)
            # self._write_addr_reg(_X_TOTAL_SIZE_H,width*2+64,height*2+64)

            self._write_addr_reg(_X_OFFSET_H, offset_x // 2, offset_y // 2)
        # self._binning = False
        # self._scale = False
        # self._write_addr_reg(_X_ADDR_ST_H,0,0)
        # self._write_addr_reg(_X_ADDR_END_H,320,240)
        # self._write_addr_reg(_X_OUTPUT_SIZE_H,320,240)
        # self._write_addr_reg(_X_TOTAL_SIZE_H,320,240)
        # self._write_addr_reg(_X_OFFSET_H,0,0)
        ...
        if self.colorspace == OV5640_COLOR_JPEG:
            ...
            if size > OV5640_SIZE_HVGA:
                self._set_pll(False,40,1,2,False,1,True,2)
            else:
                self._set_pll(False, 40, 1, 1, False, 1, True, 4)
    ...
    def _set_pll(  # pylint: disable=too-many-arguments
        ...
        self._write_register(0x460C, 0x22 if pclk_manual else 0x20)
    ...
    def test_pattern(self, value: bool) -> None:
        ...
        self._write_register(_PRE_ISP_TEST_SETTING_1, (0xF8 if value else 0x00))
